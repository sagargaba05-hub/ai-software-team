...
from ai_team.domain import Capability, WorkKind, WorkProfile
from ai_team.models import PREMIUM_TRIGGERS, CodexEscalator
from ai_team.provider_registry import (
    ProviderClass,
    classify_provider,
    discover_providers,
)
...
@pytest.mark.parametrize(
    "provider,evidence,category,eligible",
    [
        ("ollama-local", {}, ProviderClass.LOCAL, True),
        (
            "community",
            {"free": True, "billing_required": False},
            ProviderClass.FREE,
            True,
        ),
        (
            "limited",
            {"free_tier": True, "no_billing": True},
            ProviderClass.FREE_TIER,
            False,
        ),
...
def test_free_tier_requires_both_evidence_and_opt_in():
    assert classify_provider(
        "limited", {"free_tier": True, "no_billing": True}, free_tier_enabled=True
    ).eligible
    assert not classify_provider(
        "limited", {"free_tier": True}, free_tier_enabled=True
    ).eligible


def test_discovery_keeps_unknown_disabled_and_detects_local():
    records = discover_providers(
        [
            {"id": "ollama/qwen", "owned_by": "ollama-local"},
            {"id": "x/y", "owned_by": "mystery"},
        ]
    )
    indexed = {item.provider: item for item in records}
    assert indexed["ollama-local"].eligible
    assert (
        indexed["mystery"].classification is ProviderClass.UNKNOWN
        and not indexed["mystery"].eligible
    )
...
def test_audit_redacts_and_preserves_prior_lines(tmp_path):
    ...
    audit.append(
        "usage", {"run_id": "one", "api_key": "do-not-write", "detail": "Bearer abc123"}
    )
    audit.append("usage", {"run_id": "two", "result": "SUCCESS"})
    lines = (
        (tmp_path / ".ai-team/logs/usage.jsonl")
...
def test_each_premium_trigger_is_eligible_but_simulation_never_executes(
    trigger, monkeypatch
):
    monkeypatch.setattr(
        "ai_team.models.subprocess.run",
        lambda *a, **k: pytest.fail("subprocess invoked"),
    )
    decision = CodexEscalator(Settings(codex_enabled=True)).decide(
        trigger, 0, simulated=True
    )
...
def _passing_state():
    ...
    state["evidence"]["checks"] = {
        name: {"run_id": "r", "status": "PASS", "reference": f"test:{name}"}
        for name in (
            "secret_hygiene",
            "scope_hygiene",
            "health_demo",
...
def test_release_gate_rejects_stale_evidence_and_open_high():
    state = _passing_state()
    state["evidence"] = {"run_id": "old"}
    assert evaluate_release(state).status.value != "RELEASE_APPROVED"
    state = _passing_state()
    state["open_findings"] = [{"severity": "HIGH", "resolved": False}]
