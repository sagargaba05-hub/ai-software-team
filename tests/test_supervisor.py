...
def test_review_failure_routes_to_correct_role():
    assert reviewer_route({"review_status": "FAIL", "review_target": "architect", "retry_count": 1}, 3) == "architect"
    assert reviewer_route({"review_status": "FAIL", "review_target": "developer", "retry_count": 1}, 3) == "developer"


def test_release_gate_requires_all_three_gates():
    base = {
        "run_id": "run-1", "evidence": {"run_id": "run-1"},
        "requirements_status": "PASS", "architecture_status": "PASS", "implementation_status": "PASS",
        "qa_status": "PASS", "review_status": "PASS", "security_status": "NOT_REQUIRED",
        "security_reason": "no security-sensitive scope detected",
        "documentation_status": "COMPLETE", "secret_hygiene_status": "PASS", "scope_hygiene_status": "PASS",
        "health_demo_status": "PASS", "open_provider_failures": False, "open_escalation_failure": False,
    }
    assert release_gate(base)
    for key in ("requirements_status", "architecture_status", "implementation_status", "qa_status", "review_status",
                "security_status", "documentation_status", "secret_hygiene_status", "scope_hygiene_status", "health_demo_status"):
        broken = dict(base)
        broken[key] = "FAIL"
...
def test_complete_graph_retries_qa_and_review(tmp_path, skills_root):
    ...
    result = Supervisor(tmp_path, settings(skills_root), model).run("Offline workflow test")
    ...
    assert '"status": "BLOCKED"' in (tmp_path / ".ai-team" / "state.json").read_text(encoding="utf-8")


def test_exactly_three_failed_checks_stop_without_fourth_or_codex(tmp_path, skills_root):
    model = FakeModelClient(qa_failures=3)
    result = Supervisor(tmp_path, settings(skills_root), model).run("Build a local parser")
    assert result["status"] == "BLOCKED"
    assert model.calls.count("tester") == 3
    assert result["codex_calls"] == 0
    assert "three attempts" in result["findings"]
...
def test_required_stage_failure_stops_immediately(tmp_path, skills_root):
    class FailingDeveloper(FakeModelClient):
        def generate(self, role, system_prompt, user_prompt):
            ...
            return "STATUS: FAIL\nError: implementation failed" if role == "developer" else super().generate(role, system_prompt, user_prompt)
...
def test_resume_from_persisted_stage(tmp_path, skills_root):
    ...
    supervisor.store.save({
        "repository": str(tmp_path), "objective": "Resume test", "stage": "documentation",
        "status": "RUNNING", "completed": ["qa", "reviewer"], "retry_count": 0,
        "codex_calls": 0, "qa_status": "PASS", "review_status": "PASS",
        "documentation_status": "NOT_RUN", "release_status": "NOT_RUN", "findings": "",
    })
