...
DOCS = ["PRODUCT.md", "REQUIREMENTS.md", "ARCHITECTURE.md", "DECISIONS.md", "TASKS.md", "TEST_PLAN.md", "SECURITY.md", "RUNBOOK.md", "CHANGELOG.md"]
...
def bootstrap_project(root: Path) -> list[Path]:
    ...
    if _write_missing(state_json, json.dumps({"status": "INITIALIZED", "stage": "bootstrap"}, indent=2) + "\n"):
        created.append(state_json)
    onboarding = root / ".ai-team" / "onboarding.json"
    if _write_missing(onboarding, json.dumps(detect_project_facts(root), indent=2) + "\n"):
